...
- [ ] Write HTTP tests using an actual temporary directory, ephemeral port and explicit Cookie / Origin / X-Atelier-Request headers:
  ```js
  ...
  ```
- [ ] Run `node --test server/account-api.test.mjs`; observe missing account API failure before implementation.
- [ ] Implement SQLite schemas, salt/hash session and password handling, encryption key persistence, validation and atomic revision writes:
  ```sql
  UPDATE documents SET value = ?, revision = revision + 1 WHERE user_id = ? AND key = ? AND revision = ?;
  ```
- [ ] Implement bounded uploads, authenticated range downloads, per-user Codex runtimes and guarded proxy; wire both Vite modes with the same API.
- [ ] Add independent restart, wrong-password, cross-user file/task, CSRF, stale-account, encrypted-at-rest, CAS, migration and invalid-input assertions; run the real HTTP suite and baseline Codex tests.
...
- [ ] Write lifecycle tests for anonymous gate, successful bootstrap, failed hydration, and logout failure without false success; observe failures with Vitest.
- [ ] Implement accessible Chinese forms with validation, pending/error states, registration availability and password change.
- [ ] Hydrate before setting authenticated, flush before logout, clear on identity switch. Broadcast only account-change notifications (not credentials) and react to expired/stale sessions.
- [ ] Run focused tests and typecheck; report exact changed paths.
...
- [ ] Add adapter tests that catch lost writes, unauthorized writes, version conflicts and accidental localStorage fallback; run them red.
- [ ] Replace business localStorage reads/writes with bootstrapped API state. Do not overwrite local legacy data or change unrelated 3D workflows.
- [ ] Warn before closing unsaved changes, prevent stale-account writes and preserve explicit file export/import.
- [ ] Run tests and standalone build; report exact paths; main worker rebuilds distributed assets last.
...
- [ ] Write Vitest tests for serial CAS saves, pending edits during an in-flight request, retry without data loss, conflict without overwrite and account epoch isolation.
- [ ] Implement in-memory snapshot + save queue; disable store auto-hydration; hydrate from server before enabling writes.
- [ ] Switch media resolution/upload/deletion and backup enumeration to authenticated server APIs. Add explicit beforeunload / saving / failed-save UI.
- [ ] Test then implement legacy discovery and migration: no write to old databases; reject nonempty remote accounts; upload referenced media then commit atomically and mark success afterwards.
- [ ] Mount account gate and migration notices, run all unit tests and typecheck.
...
- [ ] Document registration configuration, HTTPS/Secure Cookie, persisted key/database/uploads/Codex, migration and consistent stopped-service backup/recovery.
- [ ] Build Node image with both frontends and pinned Codex CLI; run non-root with named /data volume and loopback-only published port. Do not deploy it externally.
- [ ] Run `npm run test:unit`, `npm run test:codex`, `npm run typecheck`, `npm run build:monoform`, `npm run build` and new storage tests.
- [ ] Browser check fresh signup/login, create/reload canvas, migrate data, save MONOFORM, logout/login, second account isolation. Verify server restart against a disposable persisted directory.
- [ ] Review diff and security boundaries; record exact results and any environment-specific checks not performed.
