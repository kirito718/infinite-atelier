import {
  ...
  type DirectorCaptureResult,
  type DirectorControlPass,
...
const hasEnvelope = (value: unknown): value is Record<string, unknown> => {
  if (!isRecord(value)) return false;
  return (
    value.protocol === DIRECTOR_PROTOCOL &&
    value.version === DIRECTOR_PROTOCOL_VERSION &&
    (value.source === "atelier" || value.source === "monoform") &&
...
export const isDirectorCaptureResult = (value: unknown): value is DirectorCaptureResult => {
  ...
  return (
    ...
    isFiniteNumber(payload.frame) &&
    isPass(payload.pose) &&
    isPass(payload.depth)
  );
};
...
const isCameraMetadata = (value: unknown): boolean => {
  ...
  return (
    Array.isArray(value.position) && value.position.length === 3 && value.position.every(isFiniteNumber) &&
    Array.isArray(value.rotation) && value.rotation.length === 3 && value.rotation.every(isFiniteNumber) &&
    isFiniteNumber(value.focalLength) && typeof value.aspectRatio === "string"
  );
};
...
export const parseDirectorMessage = (value: unknown): DirectorMessage | null => {
  ...
  if (value.source === "monoform" && (value.type === "ready" || value.type === "monoform:ready")) {
    ...
    if (value.payload !== undefined) {
      ...
      if (
        value.payload.projectKey !== undefined &&
      ...
      if (
        ...
        (!Array.isArray(value.payload.capabilities) ||
          value.payload.capabilities.some(
            (capability) =>
              capability !== "capture-image" && capability !== "capture-pose" && capability !== "capture-depth",
          ))
      ) return null;
    }
...
export const isAllowedDirectorEvent = (
  event: MessageEvent,
